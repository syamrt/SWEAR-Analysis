...
def askQuestion():
    global questions, window, index, button, right, number_of_questions,status
    #status=0
    if(len(questions)==index+1):
        Label(window, text="Thank you for answering the questions. " + str(right) + " of 10"  + " questions answered right").pack()
        status=1
        return
    index += 1
...
def function():
    ...
    global button
    button=Button(window, text="Start", command=askQuestion).place(x=600,y=150)
